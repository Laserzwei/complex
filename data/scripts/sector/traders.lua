...
function Traders.update(timeStep)
    ...
    --local scripts = {"consumer.lua", "seller.lua", "turretfactoryseller.lua", "factory.lua", "tradingpost.lua", "planetarytradingpost.lua", "complexFactory.lua"}
    local scripts = {"complexFactory.lua"}
    local sector = Sector()
    ...
    for _, v in pairs(tradingStations) do
        ...
        for _, name in pairs(sold) do
            ...
            if err == 0 and maxAmount > 0 and amount / maxAmount > 0.6 then
                tradingPossibilities[#tradingPossibilities+1] = {tradeType = TradeType.BuyFromStation, station = station, script = script, name = name}
            end
        end
        ...
        for _, name in pairs(bought) do
            ...
            if err == 0 and maxAmount > 0 and amount / maxAmount < 0.4 then
                tradingPossibilities[#tradingPossibilities+1] = {tradeType = TradeType.SellToStation, station = station, script = script, name = name, amount = maxAmount - amount}
            end
        end
